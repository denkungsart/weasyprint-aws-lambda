...
This repository is deliberately limited to the image build, test, and release
pipeline. It does not provision AWS infrastructure or modify consuming
applications. Each deployment can adopt the image gradually and retain its
existing PDF path during validation.
...
When migrating from wkhtmltopdf, translate renderer-specific switches into
...
and failure handling are acceptable. Migration from programmatic PDF libraries
such as Prawn is a separate concern.
